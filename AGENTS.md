...
- `## What I Use` `### Hardware` / `### Software` Connect-style shields.io tiles → `uses.hardware` / `uses.software` (`name`, `badgeUrl`, `url`) rendered as logo tiles on the site. GitHub + site only — never the resume.
